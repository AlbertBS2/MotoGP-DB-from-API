...
def specific_season_riders(json_season_standings):
    """
    ...
    df_riders = pd.DataFrame(flattened_data)

    return df_riders

def all_seasons_riders(json_season_info, category_id, start_year=1949, end_year=date.today().year):
    """
    """
    list_all_seasons_riders = []
    i = 0
    ...
    for season in json_season_info:
        ...
        df_season_riders_data = specific_season_riders(json_season_standings)

        list_all_seasons_riders.append(df_season_riders_data)
        i += 1
        print(i)
            
    df_all_seasons_riders = pd.concat(list_all_seasons_riders, ignore_index=True)
    df_all_seasons_riders = df_all_seasons_riders.drop_duplicates()

    print(f'DataFrame with all seasons created')
    return df_all_seasons_riders
...
df_all_seasons_riders_motogp = all_seasons_riders(json_season_info, category_id_motogp)

# Save the df as csv
df_all_seasons_riders_motogp.to_csv(out_filename, index=False, sep=';')
